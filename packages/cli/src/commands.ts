...
// Palette CSS is opt-in (see `paletteThemes` in @stalk-ui/preset): identities
// ship generated by default; palettes need a consumer `staticCss.themes` entry.
const PALETTE_NAMES = ['blue', 'violet', 'teal', 'emerald', 'amber', 'yellow', 'orange', 'red']

const palette = (name: string) => ({
  description: `curated accent palette — opt in via staticCss.themes, apply via data-panda-theme="${name}"`,
  optIn: true,
})
...
const themes: Record<string, { description: string; optIn: boolean }> = {
  ...identityThemes,
  ...Object.fromEntries(PALETTE_NAMES.map((name) => [name, palette(name)])),
}

export const themeCommand = (name?: string) => {
  if (name === undefined) {
    console.log(`Stalk UI ships three theme identities and ${String(PALETTE_NAMES.length)} curated accent palettes:

  neutral (default) - ${identityThemes.neutral.description}
  rainbow           - ${identityThemes.rainbow.description}
  monochrome        - ${identityThemes.monochrome.description}
...
https://stalk-ui.com/en/docs/getting-started/theming`)
  ...
  if (theme !== undefined) {
    const optInNote = theme.optIn
      ? `\nGenerate its CSS first (panda.config.ts):\n  staticCss: { themes: ['${name}'] }\n`
      : ''
